...
// Graph data structures and implementations
...
template <class T> Graph<T>::Graph(unordered_map<Vertex<T>, vector<Edge<T>>, HashFunction<T>> adjcDict)
    : adjcDict(adjcDict) {}
...
template <class T> vector<Edge<T>> Graph<T>::edges() const {
  ...
  for (auto [vertex, _edges] : adjcDict) {
    copy(_edges.begin(), _edges.end(), back_inserter(result));
  }
  return result;
}
...
template <class T> int dijkstra(const Graph<T> &graph, Vertex<T> source, Vertex<T> destination) {
  ...
  auto cmp = [](const pair<int, Vertex<T>> &a, const pair<int, Vertex<T>> &b) { return a.first > b.first; };
  priority_queue<pair<int, Vertex<T>>, vector<pair<int, Vertex<T>>>, decltype(cmp)> pq(cmp);
  pq.push({0, source});

  while (!pq.empty()) {
    auto [currentDist, u] = pq.top();
    pq.pop();
    if (u == destination) return distances[destination];
...
template int dijkstra<int>(const Graph<int> &graph, Vertex<int> source, Vertex<int> destination);
template int dijkstra<char>(const Graph<char> &graph, Vertex<char> source, Vertex<char> destination);
template int dijkstra<float>(const Graph<float> &graph, Vertex<float> source, Vertex<float> destination);
template int dijkstra<double>(const Graph<double> &graph, Vertex<double> source, Vertex<double> destination);
